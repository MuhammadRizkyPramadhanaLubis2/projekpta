...
if (!$satkerId) {
    ...
    $satkers = db()->query('SELECT id, nama, role, unit FROM users WHERE status = "active" AND unit != "PTA Medan" AND role LIKE "Satker%" GROUP BY unit ORDER BY nama')->fetchAll();
                    ...
                    <tbody>
                        ...
                        <?php foreach ($satkers as $index => $satker): ?>
                            <tr>
                                ...
                                <td style="text-align: center; color: #10b981; font-weight: bold; font-size: 1.2rem;">A<br>ðŸ˜ƒ</td>
                                <td style="text-align: center; color: var(--text-muted);">Evaluasi</td>
                                <td style="text-align: center;">
                                    <form method="post" enctype="multipart/form-data" style="display: flex; flex-direction: column; gap: 5px; align-items: center;">
                                        <input type="file" name="lhe_file" style="max-width: 200px; font-size: 0.8rem;">
                                        <button type="button" class="button primary" style="padding: 4px 12px; font-size: 0.85rem;">Unggah</button>
                                    </form>
                                </td>
                            </tr>
                        <?php endforeach; ?>
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $flash = ['type' => 'success', 'message' => 'Data evaluasi berhasil disimpan.'];
}
                            ...
                            <?php foreach ($section['subsections'] as $sub): ?>
                                    ...
                                    <td>
                                        ...
                                        <select name="eval_jawaban[<?= h($sub['code']) ?>]" 
                                            ...
                                            <?php foreach ($options as $opt): ?>
                                                <option value="<?= $opt ?>" <?= $opt === $sub['evaluator_answer'] ? 'selected' : '' ?>><?= $opt ?></option>
                                            <?php endforeach; ?>
                                        </select>
                                    </td>
                                    <td style="text-align: center;">
                                        <input type="number" step="0.01" 
                                               ...
                                               value="<?= h($sub['evaluator_score']) ?>" 
                                               readonly 
                                               style="width: 80px; text-align: center; background: transparent; border: none; font-weight: bold;">
                                    </td>
                                    <td>
                                        <textarea name="eval_notes[<?= h($sub['code']) ?>]" rows="2" placeholder="Catatan evaluator..."><?= h($sub['notes']) ?></textarea>
                                    </td>
                                    <td></td>
                                </tr>
                                ...
                                <?php foreach ($sub['criteria'] as $index => $criterion): ?>
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <textarea name="criteria_notes[<?= h($sub['code']) ?>][<?= $index ?>]" rows="2" placeholder="Catatan kriteria..."><?= h($criterion['note']) ?></textarea>
                                        </td>
                                        <td>
